import productData from "./api/rent";
...
const rent = () => {
  return (
    <>
      <div>
        <h1 className="text-4xl m-10 font-bold e text-center">
        RENT PROPERTY
        </h1>
        <div className="grid m-5 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-6">
      {productData.map((product) => (
        <Link href={`/rooms/${product.id}`} key={product.id} className="relative"><div
          
          className="block hover:scale-105 duration-1000 hover:shadow-indigo-400 rounded-lg p-4 shadow-lg shadow-indigo-100"
        >
